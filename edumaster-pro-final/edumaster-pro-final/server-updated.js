...
app.post('/api/auth/teacher/login', async (req, res) => {
    try {
        const { identifier, password } = req.body; // Can be email or employee ID
        
        const user = await User.findOne({ 
            $or: [
                { email: identifier, role: 'teacher' },
                { employeeId: identifier, role: 'teacher' }
            ]
        });
        ...
        const token = jwt.sign({ userId: user._id, role: 'teacher' }, JWT_SECRET);
        res.json({ user, token, role: 'teacher' });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
app.post('/api/auth/student/login', async (req, res) => {
    try {
        ...
        res.json({ user, token, role: 'student' });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
app.post('/api/auth/accountant/login', async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(401).json({ error: 'Invalid accountant credentials' });
        }
        ...
        const token = jwt.sign({ userId: user._id, role: 'accountant' }, JWT_SECRET);
        res.json({ user, token, role: 'accountant' });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
app.post('/api/auth/logout', authenticate, async (req, res) => {
    try {
        ...
        // For now, client-side will handle removing the token
